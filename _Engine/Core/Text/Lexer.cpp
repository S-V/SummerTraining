...
static inline bool IsLetter( char c ) {
	...
	void InitCharTable()
	{
		...
		}
		{
			...
			gs_CharMap[ '.' ] = CharIsPunctuation|CharIsPeriod;
		}
	...
	#define ADD_FLAG( CHAR_FLAG )\
			...
			file.Write(charInfo.ToPtr(), charInfo.Length());
		}
		return 0;
	}
#endif
...
bool Lexer::ReadToken( Token &newToken )
{
	return this->ReadNextTokenInternal( newToken );
}

bool Lexer::PeekToken( Token &nextToken )
{
...
bool Lexer::ReadNextTokenInternal( Token &token )
{
	...
	LexError(*this, "unknown token: '%s'", token.text.c_str());
	mxASSERT(false);
	return false;
}
...
bool Lexer::ExpectNextCharInternal( char c )
{
	...
	{
		LexError(*this, "couldn't read expected '%c' : end of file reached", c);
		return false;
	}
	...
	if( currChar != c )
	{
		LexError(*this, "expected '%c', but got '%c'", c, currChar);
		return false;
	}
...
================
*/
bool Lexer::ReadLine( Token & token )
{
...
{
	return IsEndOfFile( CurrChar() ) || ((m_curr - m_source) >= m_length);
}
...
bool Lexer::ReadName( Token &token )
{
	...
	while( IsIdentifierBodyChar( CurrChar(), m_options ) )
	{
		if( m_curr - startPos > mxCOUNT_OF(charBuffer)-1 )
		{
			LexError( *this, "identifier '%s' is too long (max allowed length is %u characters)", charBuffer, int(mxCOUNT_OF(charBuffer)-1) );
			return false;
		}
		*writePtr++ = CurrChar();
		ReadChar();
...
bool Lexer::ReadString( Token &token )
{
	...
	while ( CurrChar() != '\"' && !EndOfFile() )
	{
		if( CurrChar() == '\n' )
		{
			LexError(*this, "new line in string constant");
			return false;
		}
...
// and returns its type ( L_INT32, L_UINT32, ... ).
...
bool Lexer::ReadNumber( Token &token )
{
	...
	{
		LexError(*this, "more than one dot (%u) in a number", numDots);
	}

	return false;
}

bool Lexer::ReadInteger( Token &token )
{
	char	charBuffer[MAX_TOKEN_LENGTH];
	mxZERO_OUT(charBuffer);
...
bool Lexer::ReadFloat( Token &token )
{
	char	charBuffer[MAX_TOKEN_LENGTH];
	mxZERO_OUT(charBuffer);
...
bool Lexer::ReadHexadecimal( Token &token )
{
...
bool Lexer::ReadOctalInteger( Token &token )
{
	UNDONE;
...
bool Lexer::ReadBinaryInteger( Token &token )
{
	UNDONE;
...
// Punctuation & Operators
//
bool Lexer::ParsePunctuation( Token &token )
{
	const int lastLine = m_currentLine;
	const int lastColumn = m_currentColumn;
	...
	if( CurrChar() == '/' )
	{
		...
		else if ( PeekChar() == '*' )
		{
			...
			while( true )
			{
				...
				if ( CurrChar() == '*' )
				{
					...
					{
						if ( PeekChar() == '*' ) {
							LexWarning( *this, "nested comment" );
						}
		...
		else if ( PeekChar() == '+' ) 
		{
			...
			while( nestLevel > 0 )
			{
				...
				if( EndOfFile() ) {
					if( nestLevel > 0 ) {
						LexWarning( "unterminated /+ +/ comment; end of file reached." );
					}
					return false;
				}
