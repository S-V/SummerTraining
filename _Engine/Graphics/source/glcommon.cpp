...
namespace llgl
{
	...
	const char* g_attributeNames[VertexAttribute::Count] =
	{
		"a_position",
	...
	{
		for( UINT i = 0; i < VertexAttribute::Count; i++ ) {
			if( !strcmp(g_attributeNames[i], attribName) ) {
				return (VertexAttribute::Enum) i;
			}
		}
		ptERROR("Unknown vertex attribute: '%s'\n", attribName);
		return (VertexAttribute::Enum) -1;
	}
	...
	GLenum ConvertAttribTypeGL( AttributeType::Enum attribType )
	{
		...
		case AttributeType::Half :		return GL_HALF_FLOAT_ARB;
	...
	GLenum ConvertShaderTypeGL( EShaderType shaderType )
	{
		switch( shaderType )
		{
		case ShaderVertex :		return GL_VERTEX_SHADER;
		case ShaderGeometry :	return GL_GEOMETRY_SHADER;
		case ShaderFragment :	return GL_FRAGMENT_SHADER;		
	...
	GLenum ConvertComparisonFunctionGL( ComparisonFunc::Enum depthFunc )
	{
		...
		case ComparisonFunc::Not_Equal :		return GL_NOTEQUAL;
		case ComparisonFunc::Less_Equal :		return GL_LEQUAL;
		case ComparisonFunc::Greater_Equal :	return GL_GEQUAL;
			mxNO_SWITCH_DEFAULT;
		}
		return -1;
	}
	...
	GLenum ConvertStencilOpGL( StencilOp::Enum stencilOp )
	{
		...
		case StencilOp::INCR_SAT :	return GL_INCR;
		case StencilOp::DECR_SAT :	return GL_DECR;
	...
	GLenum ConvertBlendFactorGL( BlendFactor::Enum blendFactor )
	{
		...
		case BlendFactor::SRC1_ALPHA		:	return GL_SOURCE1_ALPHA;
		case BlendFactor::BLEND_FACTOR		:	return GL_CONSTANT_COLOR;

		case BlendFactor::INV_SRC_COLOR		:	return GL_ONE_MINUS_SRC_COLOR;
		case BlendFactor::INV_DST_COLOR		:	return GL_ONE_MINUS_DST_COLOR;
		case BlendFactor::INV_SRC_ALPHA		:	return GL_ONE_MINUS_SRC_ALPHA;
		case BlendFactor::INV_DST_ALPHA		:	return GL_ONE_MINUS_DST_ALPHA;
		case BlendFactor::INV_SRC1_COLOR	:	return GL_ONE_MINUS_SRC1_COLOR;
		case BlendFactor::INV_SRC1_ALPHA	:	return GL_TRIANGLE_STRIP;
		case BlendFactor::INV_BLEND_FACTOR	:	return GL_ONE_MINUS_SRC1_ALPHA;
	...
	{
		return MakeShader(ShaderVertex, sourceCode, sourceLength, shader);
	}
	ERet CreateFragmentShader( const GLchar* sourceCode, GLint sourceLength, GLuint &shader )
	{
		return MakeShader(ShaderFragment, sourceCode, sourceLength, shader);
	}
	...
	ERet MakeProgram( GLuint vertexShader, GLuint fragmentShader, GLuint &program )
	{
		...
		{
			const GLuint semantics = VertexAttribute::Position + attribIndex;
	...
	const TextureFormatGL gs_textureFormats[PixelFormat::MAX] =
	{
...
ERet WGLContext::Setup( HWND hWnd )
{
	...
	glDebugMessageCallbackARB ((GLDEBUGPROCARB)&llgl::My_OpenGL_Error_Callback, NULL);
	...
	DBGOUT("UBOs: VS=%u, GS=%u, PS=%u", maxVertexShaderUniformBlocks, maxGeometryShaderUniformBlocks, maxFragmentShaderUniformBlocks);
...
void WGLContext::GetRect(Viewport &area)
{
	RECT	rect;
