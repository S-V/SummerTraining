...
#pragma hdrstop
//#include <pstdint.h>
#include <Core/bx/bx.h>
#include <Core/bx/float4_t.h>
#include <Core/bx/readerwriter.h>
...
namespace bgfx
{
	...
	mxSTATIC_ASSERT(PixelFormat::MAX == mxCOUNT_OF(s_imageBlockInfo) );
	...
	mxSTATIC_ASSERT(PixelFormat::MAX == mxCOUNT_OF(s_textureFormatName) );

	bool isCompressed(PixelFormat::Enum _format)
	{
		return _format < PixelFormat::Unknown;
	}

	bool isColor(PixelFormat::Enum _format)
	{
		return _format > PixelFormat::Unknown
//			&& _format < PixelFormat::UnknownDepth
			;
	}

	bool isDepth(PixelFormat::Enum _format)
	{
		return 1//_format > PixelFormat::UnknownDepth
			&& _format < PixelFormat::MAX
			;
	}

	uint8_t getBitsPerPixel(PixelFormat::Enum _format)
	{
		return s_imageBlockInfo[_format].bitsPerPixel;
	}

	const ImageBlockInfo& getBlockInfo(PixelFormat::Enum _format)
	{
		return s_imageBlockInfo[_format];
	}

	uint8_t getBlockSize(PixelFormat::Enum _format)
	{
		return s_imageBlockInfo[_format].blockSize;
	}

	const char* getName(PixelFormat::Enum _format)
	{
...
#define DDSCAPS2_VOLUME             0x00200000
	...
	{
		uint32_t m_format;
		PixelFormat::Enum m_textureFormat;
	};
	...
	static TranslateDdsFormat s_translateDdsFourccFormat[] =
	{
		...
		{ DDS_A16B16G16R16,          PixelFormat::RGBA16  },
		{ DDS_A16B16G16R16F,         PixelFormat::RGBA16F },
		{ DDPF_RGB|DDPF_ALPHAPIXELS, PixelFormat::BGRA8   },
		{ DDPF_INDEXED,              PixelFormat::R8      },
		{ DDPF_LUMINANCE,            PixelFormat::R8      },
		{ DDPF_ALPHA,                PixelFormat::R8      },
		{ DDS_R16F,                  PixelFormat::R16F    },
		{ DDS_R32F,                  PixelFormat::R32F    },
		{ DDS_A8L8,                  PixelFormat::RG8     },
		{ DDS_G16R16,                PixelFormat::RG16    },
		{ DDS_G16R16F,               PixelFormat::RG16F   },
		{ DDS_G32R32F,               PixelFormat::RG32F   },
		{ DDS_A8R8G8B8,              PixelFormat::BGRA8   },
		{ DDS_A16B16G16R16,          PixelFormat::RGBA16  },
		{ DDS_A16B16G16R16F,         PixelFormat::RGBA16F },
		{ DDS_A32B32G32R32F,         PixelFormat::RGBA32F },
		{ DDS_R5G6B5,                PixelFormat::R5G6B5  },
		{ DDS_A4R4G4B4,              PixelFormat::RGBA4   },
		{ DDS_A1R5G5B5,              PixelFormat::RGB5A1  },
		{ DDS_A2B10G10R10,           PixelFormat::RGB10A2 },
	};

	static TranslateDdsFormat s_translateDxgiFormat[] =
	{
		{ DDS_FORMAT_BC1_UNORM,          PixelFormat::BC1     },
		{ DDS_FORMAT_BC2_UNORM,          PixelFormat::BC2     },
		{ DDS_FORMAT_BC3_UNORM,          PixelFormat::BC3     },
		{ DDS_FORMAT_BC4_UNORM,          PixelFormat::BC4     },
		{ DDS_FORMAT_BC5_UNORM,          PixelFormat::BC5     },
		{ DDS_FORMAT_BC6H_SF16,          PixelFormat::BC6H    },
		{ DDS_FORMAT_BC7_UNORM,          PixelFormat::BC7     },

		{ DDS_FORMAT_R8_UNORM,           PixelFormat::R8      },
		{ DDS_FORMAT_R16_UNORM,          PixelFormat::R16     },
		{ DDS_FORMAT_R16_FLOAT,          PixelFormat::R16F    },
		{ DDS_FORMAT_R32_UINT,           PixelFormat::R32     },
		{ DDS_FORMAT_R32_FLOAT,          PixelFormat::R32F    },
		{ DDS_FORMAT_R8G8_UNORM,         PixelFormat::RG8     },
		{ DDS_FORMAT_R16G16_UNORM,       PixelFormat::RG16    },
		{ DDS_FORMAT_R16G16_FLOAT,       PixelFormat::RG16F   },
		{ DDS_FORMAT_R32G32_UINT,        PixelFormat::RG32    },
		{ DDS_FORMAT_R32G32_FLOAT,       PixelFormat::RG32F   },
		{ DDS_FORMAT_B8G8R8A8_UNORM,     PixelFormat::BGRA8   },
		{ DDS_FORMAT_R16G16B16A16_UNORM, PixelFormat::RGBA16  },
		{ DDS_FORMAT_R16G16B16A16_FLOAT, PixelFormat::RGBA16F },
		{ DDS_FORMAT_R32G32B32A32_UINT,  PixelFormat::RGBA32  },
		{ DDS_FORMAT_R32G32B32A32_FLOAT, PixelFormat::RGBA32F },
		{ DDS_FORMAT_B5G6R5_UNORM,       PixelFormat::R5G6B5  },
		{ DDS_FORMAT_B4G4R4A4_UNORM,     PixelFormat::RGBA4   },
		{ DDS_FORMAT_B5G5R5A1_UNORM,     PixelFormat::RGB5A1  },
		{ DDS_FORMAT_R10G10B10A2_UNORM,  PixelFormat::RGB10A2 },
	};
	...
	{
		{  8, { 0x000000ff, 0x00000000, 0x00000000, 0x00000000 }, PixelFormat::R8      },
		{ 16, { 0x0000ffff, 0x00000000, 0x00000000, 0x00000000 }, PixelFormat::R16     },
		{ 16, { 0x00000f00, 0x000000f0, 0x0000000f, 0x0000f000 }, PixelFormat::RGBA4   },
		{ 16, { 0x0000f800, 0x000007e0, 0x0000001f, 0x00000000 }, PixelFormat::R5G6B5  },
		{ 16, { 0x00007c00, 0x000003e0, 0x0000001f, 0x00008000 }, PixelFormat::RGB5A1  },
		{ 32, { 0x00ff0000, 0x0000ff00, 0x000000ff, 0xff000000 }, PixelFormat::BGRA8   },
		{ 32, { 0x00ff0000, 0x0000ff00, 0x000000ff, 0x00000000 }, PixelFormat::BGRA8   },
		{ 32, { 0x000003ff, 0x000ffc00, 0x3ff00000, 0xc0000000 }, PixelFormat::RGB10A2 },
		{ 32, { 0x0000ffff, 0xffff0000, 0x00000000, 0x00000000 }, PixelFormat::RG16    },
		{ 32, { 0xffffffff, 0x00000000, 0x00000000, 0x00000000 }, PixelFormat::R32     },
	};
	...
	bool imageParseDds(ImageContainer& _imageContainer, bx::ReaderSeekerI* _reader)
	{
		...
		if (dxgiFormat == 0)
		{
			...
			{
				for (uint32_t ii = 0; ii < mxCOUNT_OF(s_translateDdsFourccFormat); ++ii)
				{
			...
			{
				for (uint32_t ii = 0; ii < mxCOUNT_OF(s_translateDdsPixelFormat); ++ii)
				{
		...
		{
			for (uint32_t ii = 0; ii < mxCOUNT_OF(s_translateDxgiFormat); ++ii)
			{
		...
		return PixelFormat::Unknown != format;
	}
...
#define KTX_RGB10_A2                                  0x8059
	...
	{
		{ KTX_COMPRESSED_RGBA_S3TC_DXT1_EXT,             PixelFormat::BC1     },
		{ KTX_COMPRESSED_RGBA_S3TC_DXT3_EXT,             PixelFormat::BC2     },
		{ KTX_COMPRESSED_RGBA_S3TC_DXT5_EXT,             PixelFormat::BC3     },
		{ KTX_COMPRESSED_LUMINANCE_LATC1_EXT,            PixelFormat::BC4     },
		{ KTX_COMPRESSED_LUMINANCE_ALPHA_LATC2_EXT,      PixelFormat::BC5     },
		{ KTX_COMPRESSED_RGB_BPTC_SIGNED_FLOAT_ARB,      PixelFormat::BC6H    },
		{ KTX_COMPRESSED_RGBA_BPTC_UNORM_ARB,            PixelFormat::BC7     },
		{ KTX_ETC1_RGB8_OES,                             PixelFormat::ETC1    },
		{ KTX_COMPRESSED_RGB8_ETC2,                      PixelFormat::ETC2    },
		{ KTX_COMPRESSED_RGBA8_ETC2_EAC,                 PixelFormat::ETC2A   },
		{ KTX_COMPRESSED_RGB8_PUNCHTHROUGH_ALPHA1_ETC2,  PixelFormat::ETC2A1  },
		{ KTX_COMPRESSED_RGB_PVRTC_2BPPV1_IMG,           PixelFormat::PTC12   },
		{ KTX_COMPRESSED_RGBA_PVRTC_2BPPV1_IMG,          PixelFormat::PTC12A  },
		{ KTX_COMPRESSED_RGB_PVRTC_4BPPV1_IMG,           PixelFormat::PTC14   },
		{ KTX_COMPRESSED_RGBA_PVRTC_4BPPV1_IMG,          PixelFormat::PTC14A  },
		{ KTX_COMPRESSED_RGBA_PVRTC_2BPPV2_IMG,          PixelFormat::PTC22   },
		{ KTX_COMPRESSED_RGBA_PVRTC_4BPPV2_IMG,          PixelFormat::PTC24   },
		{ KTX_R8,                                        PixelFormat::R8      },
		{ KTX_RGBA16,                                    PixelFormat::RGBA16  },
		{ KTX_RGBA16F,                                   PixelFormat::RGBA16F },
		{ KTX_R32UI,                                     PixelFormat::R32     },
		{ KTX_R32F,                                      PixelFormat::R32F    },
		{ KTX_RG8,                                       PixelFormat::RG8     },
		{ KTX_RG16,                                      PixelFormat::RG16    },
		{ KTX_RG16F,                                     PixelFormat::RG16F   },
		{ KTX_RG32UI,                                    PixelFormat::RG32    },
		{ KTX_RG32F,                                     PixelFormat::RG32F   },
		{ KTX_BGRA,                                      PixelFormat::BGRA8   },
		{ KTX_RGBA16,                                    PixelFormat::RGBA16  },
		{ KTX_RGBA16F,                                   PixelFormat::RGBA16F },
		{ KTX_RGBA32UI,                                  PixelFormat::RGBA32  },
		{ KTX_RGBA32F,                                   PixelFormat::RGBA32F },
		{ KTX_RGB565,                                    PixelFormat::R5G6B5  },
		{ KTX_RGBA4,                                     PixelFormat::RGBA4   },
		{ KTX_RGB5_A1,                                   PixelFormat::RGB5A1  },
		{ KTX_RGB10_A2,                                  PixelFormat::RGB10A2 },
	};
#if 0
	...
	bool imageParseKtx(ImageContainer& _imageContainer, bx::ReaderSeekerI* _reader)
	{
		...
		for (uint32_t ii = 0; ii < mxCOUNT_OF(s_translateKtxFormat); ++ii)
		{
...
#define PVR3_CHANNEL_TYPE_FLOAT UINT32_C(12)
	...
	{
		{ PVR3_PVRTC1_2BPP_RGB,  PVR3_CHANNEL_TYPE_ANY,   PixelFormat::PTC12   },
		{ PVR3_PVRTC1_2BPP_RGBA, PVR3_CHANNEL_TYPE_ANY,   PixelFormat::PTC12A  },
		{ PVR3_PVRTC1_4BPP_RGB,  PVR3_CHANNEL_TYPE_ANY,   PixelFormat::PTC14   },
		{ PVR3_PVRTC1_4BPP_RGBA, PVR3_CHANNEL_TYPE_ANY,   PixelFormat::PTC14A  },
		{ PVR3_PVRTC2_2BPP_RGBA, PVR3_CHANNEL_TYPE_ANY,   PixelFormat::PTC22   },
		{ PVR3_PVRTC2_4BPP_RGBA, PVR3_CHANNEL_TYPE_ANY,   PixelFormat::PTC24   },
		{ PVR3_ETC1,             PVR3_CHANNEL_TYPE_ANY,   PixelFormat::ETC1    },
		{ PVR3_DXT1,             PVR3_CHANNEL_TYPE_ANY,   PixelFormat::BC1     },
		{ PVR3_DXT2,             PVR3_CHANNEL_TYPE_ANY,   PixelFormat::BC2     },
		{ PVR3_DXT3,             PVR3_CHANNEL_TYPE_ANY,   PixelFormat::BC2     },
		{ PVR3_DXT4,             PVR3_CHANNEL_TYPE_ANY,   PixelFormat::BC3     },
		{ PVR3_DXT5,             PVR3_CHANNEL_TYPE_ANY,   PixelFormat::BC3     },
		{ PVR3_BC4,              PVR3_CHANNEL_TYPE_ANY,   PixelFormat::BC4     },
		{ PVR3_BC5,              PVR3_CHANNEL_TYPE_ANY,   PixelFormat::BC5     },
		{ PVR3_R8,               PVR3_CHANNEL_TYPE_ANY,   PixelFormat::R8      },
		{ PVR3_R16,              PVR3_CHANNEL_TYPE_ANY,   PixelFormat::R16     },
		{ PVR3_R16,              PVR3_CHANNEL_TYPE_FLOAT, PixelFormat::R16F    },
		{ PVR3_R32,              PVR3_CHANNEL_TYPE_ANY,   PixelFormat::R32     },
		{ PVR3_R32,              PVR3_CHANNEL_TYPE_FLOAT, PixelFormat::R32F    },
		{ PVR3_RG8,              PVR3_CHANNEL_TYPE_ANY,   PixelFormat::RG8     },
		{ PVR3_RG16,             PVR3_CHANNEL_TYPE_ANY,   PixelFormat::RG16    },
		{ PVR3_RG16,             PVR3_CHANNEL_TYPE_FLOAT, PixelFormat::RG16F   },
		{ PVR3_RG32,             PVR3_CHANNEL_TYPE_ANY,   PixelFormat::RG16    },
		{ PVR3_RG32,             PVR3_CHANNEL_TYPE_FLOAT, PixelFormat::RG32F   },
		{ PVR3_BGRA8,            PVR3_CHANNEL_TYPE_ANY,   PixelFormat::BGRA8   },
		{ PVR3_RGBA16,           PVR3_CHANNEL_TYPE_ANY,   PixelFormat::RGBA16  },
		{ PVR3_RGBA16,           PVR3_CHANNEL_TYPE_FLOAT, PixelFormat::RGBA16F },
		{ PVR3_RGBA32,           PVR3_CHANNEL_TYPE_ANY,   PixelFormat::RGBA32  },
		{ PVR3_RGBA32,           PVR3_CHANNEL_TYPE_FLOAT, PixelFormat::RGBA32F },
		{ PVR3_RGB565,           PVR3_CHANNEL_TYPE_ANY,   PixelFormat::R5G6B5  },
		{ PVR3_RGBA4,            PVR3_CHANNEL_TYPE_ANY,   PixelFormat::RGBA4   },
		{ PVR3_RGBA51,           PVR3_CHANNEL_TYPE_ANY,   PixelFormat::RGB5A1  },
		{ PVR3_RGB10A2,          PVR3_CHANNEL_TYPE_ANY,   PixelFormat::RGB10A2 },
	};
	...
	bool imageParsePvr3(ImageContainer& _imageContainer, bx::ReaderSeekerI* _reader)
	{
		...
		for (uint32_t ii = 0; ii < mxCOUNT_OF(s_translatePvr3Format); ++ii)
		{
	...
	bool imageGetRawData(const ImageContainer& _imageContainer, uint8_t _side, uint8_t _lod, const void* _data, uint32_t _size, ImageMip& _mip)
	{
		...
		PixelFormat::Enum type = PixelFormat::Enum(_imageContainer.m_format);
