...
mxSWIPED("parts stolen from Clang: http://llvm.org/svn/llvm-project/cfe/trunk/lib/Lex/Lexer.cpp");
...
//static inline char* CurrentString( const Parser& _parser )
//{
//	return _parser.buffer + _parser.position;
//}
...
static void DebugPrintState( const Parser& _parser )
{
	...
	//const Node* items = _parser.nodes->ToPtr();
...
static inline void InsertNullCharacter( Parser & _parser, UINT32 _offset )
{
...
static UINT32 ParseListBody( Scope & _parent, Parser & _parser, Allocator & _allocator );
static UINT32 ParseArrayBody( Scope & _parent, Parser & _parser, Allocator & _allocator );
static UINT32 ParseObjectBody( Scope & _parent, Parser & _parser, Allocator & _allocator );

static Node* ParseList( Parser & _parser, Allocator & _allocator )
{
...
static Node* ParseArray( Parser & _parser, Allocator & _allocator )
{
...
static Node* ParseObject( Parser & _parser, Allocator & _allocator )
{
...
static Node* ParseNumber( Parser & _parser, Allocator & _allocator )
{
	char *	start = _parser.buffer + _parser.position;
	char *	end;
	double	f = faster_atof(start, end);

	const UINT32 charsRead = UINT32(end - start);
	if( !charsRead ) {
...
static Node* ParseString( Parser & _parser, Allocator & _allocator )
{
	ReadChar( _parser );	// consume '\''

	UINT32	stringOffset = _parser.position;
	...
	const UINT32 nullPosition = smallest(_parser.position, _parser.length-1);
...
static Node* ParseNil( Parser & _parser, Allocator & _allocator )
{
...
static Node* ParseValue( Parser & _parser, Allocator & _allocator )
{
	if( !SkipWhitespaces( _parser ) ) {
		return nil;
	}
...
static UINT32 ReadIdentifier( Parser & _parser )
{
	const UINT32 start = _parser.position;
...
static Node* ParseAssignment( Parser & _parser, Allocator & _allocator )
{
	...
	{
		const UINT32 start = _parser.position;
		const UINT32 length = ReadIdentifier( _parser );
		ReadExpectedCharacter( _parser, '=' );
		...
		if( bHashed ) {
			valueNode->tag.hash = GetDynamicStringHash( valueNode->name, length );
		}
		return valueNode;
	}
						...
						)
{
	...
	else
	{
		...
		ParseObjectBody( root, _parser, _allocator );
