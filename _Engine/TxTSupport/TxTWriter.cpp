...
namespace SON
{
	...
	static void Print_Indent( const UINT32 _depth, TextStream &_stream )
	{
		_stream << '\n';
	...
	ERet DumpValueToStream( const Node* _node, const UINT32 _depth, TextStream &_stream, int _index = -1 )
	{
		String32 valueIndex;
		if(_index != -1) {
			Str::SPrintF( valueIndex, "[%d] ", _index);
		}
		...
		case TypeTag_List :
			{
				const UINT32 count = _node->value.l.size;
				if( count )
				{
					_stream << Chars("(");
					...
					else
					{
						...
						const UINT32 listDepth = _depth + 1;
		...
		case TypeTag_Array :
			{
				const UINT32 count = _node->value.a.size;
				if( count )
				{
					_stream << Chars("[");
					...
					const UINT32 arrayDepth = _depth + 1;
		...
		case TypeTag_Object :
			{
				const UINT32 count = _node->value.o.size;
				if( count )
				{
					_stream << Chars("{");
					...
					const UINT32 objectDepth = _depth + 1;
	...
	ERet WriteToStream( const Node* _root, AStreamWriter &_stream )
	{
		//ScopedTimer	timer( "SON::WriteToStream" );
		TextStream	writer(_stream);
