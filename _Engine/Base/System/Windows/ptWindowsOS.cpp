...
{
	OutInfo = g_Win32System.systemInfo;
}
...
void ConditionVariable::Wait( const SpinWait& CS )
{
	::SleepConditionVariableCS( &m_CV, c_cast(PCRITICAL_SECTION) &CS.m_CS, INFINITE );
}
...
bool Thread::Initialize( const CInfo& cInfo )
{
	...
	mxSWIPED("comments from Phyre Engine");
...
void ptLogMessage( ELogLevel level, const char* fmt, ... )
{
	...
	va_list	args;
	va_start( args, fmt );
	mxGetLog().PrintV( level, fmt, args );
	va_end( args );
}
