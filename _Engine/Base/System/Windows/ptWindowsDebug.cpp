...
struct WindowsExceptionCallback : AExceptionCallback
{
	...
	virtual void OnException() override
	{
		...
		{
			mxGetLog().VWrite(LL_Error, m_stackTrace.c_str(), m_stackTrace.size());
			::MessageBoxA(NULL, m_stackTrace.c_str(), "ERROR", MB_OK);
		}
	}
};
WindowsExceptionCallback g_defaultExceptionCallback;

AExceptionCallback* g_exceptionCallback = &g_defaultExceptionCallback;
...
static LONG WINAPI ExceptionCallback( EXCEPTION_POINTERS* exceptionInfo )
{
	...
	if( g_exceptionCallback )
	{
		...
		int options = StackWalker::RetrieveSymbol|StackWalker::RetrieveLine;
