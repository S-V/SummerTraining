...
//
UINT32	mxGetClockTicks();	// Retrieves the number of milliseconds that have elapsed since the system was started, up to 49.7 days.
UINT32	mxGetClockTicksPerSecond();	// CPU frequency in Hertz
...
int AtomicIncrement( AtomicInt& var );

// returns the resulting decremented value
int AtomicDecrement( AtomicInt& var );

// returns the resulting incremented value
int AtomicAdd( AtomicInt& var, int add );
...
//
int AtomicExchange( AtomicInt* dest, int value );
...
//
int AtomicCompareExchange( AtomicInt* dest, int comparand, int exchange );
...
void YieldHardwareThread(); // Switch to another hardware thread (on the same core)
void YieldSoftwareThread(); // Switch to another thread
void YieldToOS();	//Switch to the OS kernel.
void YieldToOSAndSleep();//Switch to the OS kernel and block for at least 1ms.
...
enum ELogLevel
{
	LL_Trace,	// An extremely detailed and verbose information.
	LL_Debug,	// A detailed information on the flow through the system.
	LL_Info,	// An informational message.
	LL_Warn,	// A warning, used for non critical failures.
	LL_Error,	// A normal error, used for potential serious failures.
	LL_Fatal	// A critical alert, used for failures which are threats to stability.
};
void ptLogMessage( ELogLevel level, const char* fmt, ... );
