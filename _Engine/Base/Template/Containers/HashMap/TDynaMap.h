...
public:
	...
	{
		mxASSERT(tableSize > 1 && IsPowerOfTwoU(tableSize));
		mxASSERT(nil == mTable && !mNumEntries);
...
public_internal:
	...
	public:
		mxFORCEINLINE Node()
			: next( nil )
		{}
		mxFORCEINLINE Node( const KEY& k, const VALUE& v, Node* next )
			: key( k ), value( v )
			, next( next )
		{}
	};
...
public:	// Iterators, algorithms, ...
	...
	class Iterator {
	public:
		mxINLINE Iterator( TDynaMap& map )
			: mMap( map )
			, mNode( map.mTable[0] )
			, mBucket( 0 )
		{
			mxASSERT(!map.IsEmpty());
			FindNextNode();
		}
...
private:
	...
	mxFORCEINLINE Node* AllocNode(  const KEY& k, const VALUE& v, Node* next )
	{
		void* mem = Allocate( sizeof(Node) );
		return new(mem) Node( k, v, next );
	}
	mxFORCEINLINE void DestroyNode( Node* ptr )
	{
		ptr->~Node();
		Free( ptr );
	}
	mxFORCEINLINE void* Allocate( size_t bytes )
	{
		return F_GetMemoryManager( mMemory )->Allocate( bytes );
	}
	mxFORCEINLINE void Free( void* ptr )
	{
