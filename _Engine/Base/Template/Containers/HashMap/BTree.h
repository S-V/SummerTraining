...
template< typename objType, typename keyType, INT maxChildrenPerNode >
mxFORCEINLINE mxBTree<objType,keyType,maxChildrenPerNode>::mxBTree( void ) {
	mxASSERT( maxChildrenPerNode >= 4 );
	root = NULL;
}
...
template< typename objType, typename keyType, INT maxChildrenPerNode >
mxFORCEINLINE void mxBTree<objType,keyType,maxChildrenPerNode>::Init( void ) {
	root = AllocNode();
}

template< typename objType, typename keyType, INT maxChildrenPerNode >
mxFORCEINLINE void mxBTree<objType,keyType,maxChildrenPerNode>::Shutdown( void ) {
	nodeAllocator.Shutdown();
	root = NULL;
}

template< typename objType, typename keyType, INT maxChildrenPerNode >
mxFORCEINLINE mxBTreeNode<objType,keyType> *mxBTree<objType,keyType,maxChildrenPerNode>::Add( objType *object, keyType key ) {
...
template< typename objType, typename keyType, INT maxChildrenPerNode >
mxFORCEINLINE void mxBTree<objType,keyType,maxChildrenPerNode>::Remove( mxBTreeNode<objType,keyType> *node ) {
	mxBTreeNode<objType,keyType> *parent;

	mxASSERT( node->object != NULL );
...
template< typename objType, typename keyType, INT maxChildrenPerNode >
mxFORCEINLINE objType *mxBTree<objType,keyType,maxChildrenPerNode>::Find( keyType key ) const {
...
template< typename objType, typename keyType, INT maxChildrenPerNode >
mxFORCEINLINE objType *mxBTree<objType,keyType,maxChildrenPerNode>::FindSmallestLargerEqual( keyType key ) const {
	mxBTreeNode<objType,keyType> *node;
...
template< typename objType, typename keyType, INT maxChildrenPerNode >
mxFORCEINLINE objType *mxBTree<objType,keyType,maxChildrenPerNode>::FindLargestSmallerEqual( keyType key ) const {
	mxBTreeNode<objType,keyType> *node;
...
template< typename objType, typename keyType, INT maxChildrenPerNode >
mxFORCEINLINE mxBTreeNode<objType,keyType> *mxBTree<objType,keyType,maxChildrenPerNode>::GetRoot( void ) const {
	return root;
}

template< typename objType, typename keyType, INT maxChildrenPerNode >
mxFORCEINLINE INT mxBTree<objType,keyType,maxChildrenPerNode>::GetNodeCount( void ) const {
	return nodeAllocator.GetAllocCount();
}

template< typename objType, typename keyType, INT maxChildrenPerNode >
mxFORCEINLINE mxBTreeNode<objType,keyType> *mxBTree<objType,keyType,maxChildrenPerNode>::GetNext( mxBTreeNode<objType,keyType> *node ) const {
	if ( node->firstChild ) {
...
template< typename objType, typename keyType, INT maxChildrenPerNode >
mxFORCEINLINE mxBTreeNode<objType,keyType> *mxBTree<objType,keyType,maxChildrenPerNode>::GetNextLeaf( mxBTreeNode<objType,keyType> *node ) const {
	if ( node->firstChild ) {
...
template< typename objType, typename keyType, INT maxChildrenPerNode >
mxFORCEINLINE mxBTreeNode<objType,keyType> *mxBTree<objType,keyType,maxChildrenPerNode>::AllocNode( void ) {
...
template< typename objType, typename keyType, INT maxChildrenPerNode >
mxFORCEINLINE void mxBTree<objType,keyType,maxChildrenPerNode>::FreeNode( mxBTreeNode<objType,keyType> *node ) {
	nodeAllocator.Free( node );
}

template< typename objType, typename keyType, INT maxChildrenPerNode >
mxFORCEINLINE void mxBTree<objType,keyType,maxChildrenPerNode>::SplitNode( mxBTreeNode<objType,keyType> *node ) {
	INT i;
	...
	mxASSERT( node->parent->numChildren < maxChildrenPerNode );
...
template< typename objType, typename keyType, INT maxChildrenPerNode >
mxFORCEINLINE mxBTreeNode<objType,keyType> *mxBTree<objType,keyType,maxChildrenPerNode>::MergeNodes( mxBTreeNode<objType,keyType> *node1, mxBTreeNode<objType,keyType> *node2 ) {
	mxBTreeNode<objType,keyType> *child;

	mxASSERT( node1->parent == node2->parent );
	mxASSERT( node1->next == node2 && node2->prev == node1 );
	mxASSERT( node1->object == NULL && node2->object == NULL );
	mxASSERT( node1->numChildren >= 1 && node2->numChildren >= 1 );
...
template< typename objType, typename keyType, INT maxChildrenPerNode >
mxFORCEINLINE void mxBTree<objType,keyType,maxChildrenPerNode>::CheckTree_r( mxBTreeNode<objType,keyType> *node, INT &numNodes ) const {
	INT numChildren;
	mxBTreeNode<objType,keyType> *child;
	...
	mxASSERT( ( node == root ) || ( node->object != NULL && node->numChildren == 0 ) || ( node->numChildren >= 2 && node->numChildren <= maxChildrenPerNode ) );
	// the key of a node may never be larger than the key of it's last child
	mxASSERT( ( node->lastChild == NULL ) || ( node->key <= node->lastChild->key ) );
	...
	for ( child = node->firstChild; child; child = child->next ) {
		...
		if ( child->prev == NULL ) {
			mxASSERT( node->firstChild == child );
		} else {
			mxASSERT( child->prev->next == child );
		}
		if ( child->next == NULL ) {
			mxASSERT( node->lastChild == child );
		} else {
			mxASSERT( child->next->prev == child );
		}
		// recurse down the tree
		CheckTree_r( child, numNodes );
	}
	// the number of children should equal the number of linked children
	mxASSERT( numChildren == node->numChildren );
}

template< typename objType, typename keyType, INT maxChildrenPerNode >
mxFORCEINLINE void mxBTree<objType,keyType,maxChildrenPerNode>::CheckTree( void ) const {
	INT numNodes = 0;
	...
	mxASSERT( numNodes == nodeAllocator.GetAllocCount() );
	...
	if ( lastNode ) {
		for ( node = GetNextLeaf( lastNode ); node; lastNode = node, node = GetNextLeaf( node ) ) {
			mxASSERT( lastNode->key <= node->key );
		}
	}
}
