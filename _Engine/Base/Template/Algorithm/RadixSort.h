...
#define BX_RADIXSORT_BIT_MASK (BX_RADIXSORT_HISTOGRAM_SIZE-1)
	...
	void radixSort32(UINT32* _keys, UINT32* _tempKeys, Ty* _values, Ty* _tempValues, UINT32 _size)
	{
		UINT16 histogram[BX_RADIXSORT_HISTOGRAM_SIZE];
		UINT16 shift = 0;
		for (UINT32 pass = 0; pass < 3; ++pass)
		{
			memset(histogram, 0, sizeof(UINT16)*BX_RADIXSORT_HISTOGRAM_SIZE);
			for (UINT32 ii = 0; ii < _size; ++ii)
			{
				UINT32 key = _keys[ii];
				UINT16 index = (key>>shift)&BX_RADIXSORT_BIT_MASK;
				++histogram[index];
			}

			UINT16 offset = 0;
			for (UINT32 ii = 0; ii < BX_RADIXSORT_HISTOGRAM_SIZE; ++ii)
			{
				UINT16 count = histogram[ii];
				histogram[ii] = offset;
				offset += count;
			}
	...
	template <typename Ty>
	void radixSort64(UINT64* _keys, UINT64* _tempKeys, Ty* _values, Ty* _tempValues, UINT32 _size)
	{
		UINT16 histogram[BX_RADIXSORT_HISTOGRAM_SIZE];
		UINT16 shift = 0;
