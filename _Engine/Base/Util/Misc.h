...
public:
	static mxFORCEINLINE TYPE* GetPtrByIndex( UINT index )
	{
		//mxASSERT( IsValidIndex( index ) );
		return ms_table[ index ];
	}
	static mxFORCEINLINE UINT GetTotalCount()
	{
		return ms_table.Num();
	}
	static mxFORCEINLINE UINT GetMaxCapacity()
	{
		return MAX;
	}
	static mxFORCEINLINE bool IsValidIndex( UINT index )
	{
		return index >= 0 && index < GetTotalCount();
	}

	static mxFORCEINLINE TYPE** GetPtrArray()
	{
...
public:
	static mxFORCEINLINE TYPE* GetPtrBySortedIndex( UINT index )
	{
		mxASSERT(mIsSorted);
		return mSortedPtrArray[ index ];
	}
	static mxFORCEINLINE bool IsTableSorted()
	{
		return mIsSorted;
	}
	...
	static void SortIndices( U** source, UINT num )
	{
		if( num > 1 )
		{
			memcpy( mSortedPtrArray.ToPtr(), source, num * (sizeof source[0]) );
