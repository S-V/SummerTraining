...
void F_Util_ComposeLogFileName( String256 & logFileName )
{
	...
	Str::SPrintF( logFileName, "%s.log", exeFileName.ToPtr() );
}
...
void mxUtil_StartLogging( ALog* logger )
{
	...
	logger->PrintF( LL_Info, "Compiled with %s", mxCOMPILER_NAME);
...
//	logger->Logf( LL_Info,  "\n==================================\n" );
	...
	logger->PrintF( LL_Info, "L1 Data Cache: %u x %u KiB, %u-way associative", numCores, cpuInfo.L1_DCache_Size/numCores, cpuInfo.L1_Assoc );
	logger->PrintF( LL_Info, "L1 Instruction Cache: %u x %u KiB, %u-way associative", numCores, cpuInfo.L1_ICache_Size/numCores, cpuInfo.L1_Assoc );

	logger->PrintF( LL_Info, "L2 Cache: %u x %u KiB, %u-way associative", numCores, cpuInfo.L2_Cache_Size/numCores, cpuInfo.L2_Assoc );

	if( cpuInfo.L3_Cache_Size && cpuInfo.L3_Assoc )
	{
		logger->PrintF( LL_Info, "L3 Cache: %u KiB, %u-way associative", cpuInfo.L3_Cache_Size, cpuInfo.L3_Assoc );
	}
