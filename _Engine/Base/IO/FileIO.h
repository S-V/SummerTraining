...
typedef TBits< EFileReadFlags, UINT32 >	FileReadFlags;
...
typedef TBits< EFileWriteFlags, UINT32 >	FileWriteFlags;
...
public:
				FileReader();
	explicit	FileReader( const char* fileName, FileReadFlags flags = FileRead_DefaultFlags );
				~FileReader();

	ERet		Open( const char* fileName, FileReadFlags flags = FileRead_DefaultFlags );
...
-----------------------------------------------------------------------------
*/
class IOStreamFILE : public AStreamReader, public AStreamWriter
{
	FILE *	m_file;
public:
	IOStreamFILE();
	...
	ERet Open( const char* filename );
	void Close();

	bool IsOpen() const;

	ERet Seek( UINT32 offset ) const;
	UINT32 SeekToEnd() const;
...
ERet Util_LoadFileToBlob( const char* fileName, ByteBuffer &data, FileReadFlags flags = FileRead_DefaultFlags );
ERet Util_SaveBlobToFile( const ByteBuffer& data, const char* fileName, FileWriteFlags flags = FileWrite_DefaultFlags );

ERet Util_LoadFileToString( const char* fileName, char **fileData, UINT *fileSize );
