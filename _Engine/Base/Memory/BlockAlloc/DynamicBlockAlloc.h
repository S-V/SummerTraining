...
#include <Base/Template/Containers/HashMap/BTree.h>
...
#ifdef DYNAMIC_BLOCK_ALLOC_CHECK
	mxASSERT( block->id[0] == 0x11111111 && block->id[1] == 0x22222222 && block->id[2] == 0x33333333 && block->allocator == (void*)this );
#endif
...
template<class type, int baseBlockSize, int minBlockSize>
mxFORCEINLINE void mxDynamicBlockAlloc<type, baseBlockSize, minBlockSize>::LinkFreeInternal( mxDynamicBlock<type> *block ) {
...
template<class type, int baseBlockSize, int minBlockSize>
mxFORCEINLINE void mxDynamicBlockAlloc<type, baseBlockSize, minBlockSize>::UnlinkFreeInternal( mxDynamicBlock<type> *block ) {
...
void mxDynamicBlockAlloc<type, baseBlockSize, minBlockSize>::CheckMemory( void ) const {
	...
	for ( block = firstBlock; block != NULL; block = block->next ) {
		...
		if ( block->prev == NULL ) {
			mxASSERT( firstBlock == block );
		} else {
			mxASSERT( block->prev->next == block );
		}
		if ( block->next == NULL ) {
			mxASSERT( lastBlock == block );
		} else {
			mxASSERT( block->next->prev == block );
		}
	}
}
