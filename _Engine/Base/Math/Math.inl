...
FORCEINLINE FLOAT Math::Tan16( FLOAT a )
{
	...
	if ( fabs( a ) >= ONEFOURTH_PI ) {
		a = ( ( a < 0.0f ) ? -mxHALF_PI : mxHALF_PI ) - a;
		reciprocal = false;
	} else {
		reciprocal = true;
	}
...
FORCEINLINE FLOAT Math::ASin16( FLOAT a )
{
	...
	if ( FLOATSIGNBITSET( a ) ) {
		...
		a = ::fabs( a );
		return ( ( ( -0.0187293f * a + 0.0742610f ) * a - 0.2121144f ) * a + 1.5707288f ) * sqrt( 1.0f - a ) - mxHALF_PI;
	} else {
		if ( a >= 1.0f ) {
			return mxHALF_PI;
		}
		return mxHALF_PI - ( ( ( -0.0187293f * a + 0.0742610f ) * a - 0.2121144f ) * a + 1.5707288f ) * sqrt( 1.0f - a );
	}
#else
	return ASin( a );
#endif
...
FORCEINLINE FLOAT Math::ACos16( FLOAT a )
{
	...
	if ( FLOATSIGNBITSET( a ) ) {
		...
		a = fabs( a );
		return mxPI - ( ( ( -0.0187293f * a + 0.0742610f ) * a - 0.2121144f ) * a + 1.5707288f ) * sqrt( 1.0f - a );
	} else {
		if ( a >= 1.0f ) {
			return 0.0f;
		}
...
FORCEINLINE FLOAT Math::UByteToF( BYTE c )
{
	return Uint8ToFloat32[ c ];
}

FORCEINLINE BYTE Math::FtoB( FLOAT f )
{
#if MX_USE_ASM
	return asmF32toU8( f );
#elif MX_MATH_USE_ID_APPROX
...
FORCEINLINE DOUBLE Math::PerlinNoise( INT x, INT y, INT random )
{
	DOUBLE result;
