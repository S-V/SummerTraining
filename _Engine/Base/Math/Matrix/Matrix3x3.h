...
public:
	...
	//for reflection
	mxREFLECTION_IS_MY_FRIEND(Matrix3);

private:
	Vec3D		mColumns[ 3 ];	// stored in column-major order
};

mxDECLARE_STRUCT( Matrix3 );

mxFORCEINLINE Matrix3::Matrix3( void ) {
}

mxFORCEINLINE Matrix3::Matrix3( const Vec3D &x, const Vec3D &y, const Vec3D &z ) {
...
mxFORCEINLINE Matrix3::Matrix3( const FLOAT xx, const FLOAT xy, const FLOAT xz, const FLOAT yx, const FLOAT yy, const FLOAT yz, const FLOAT zx, const FLOAT zy, const FLOAT zz ) {
...
mxFORCEINLINE Matrix3::Matrix3( const FLOAT src[ 3 ][ 3 ] ) {
	memcpy( mColumns, src, 3 * 3 * sizeof( FLOAT ) );
}

mxFORCEINLINE const Vec3D &Matrix3::operator[]( INT index ) const {
	//mxASSERT( ( index >= 0 ) && ( index < 3 ) );
	return mColumns[ index ];
}

mxFORCEINLINE Vec3D &Matrix3::operator[]( INT index ) {
	//mxASSERT( ( index >= 0 ) && ( index < 3 ) );
	return mColumns[ index ];
}

mxFORCEINLINE Matrix3 Matrix3::operator-() const {
...
mxFORCEINLINE Vec3D Matrix3::operator*( const Vec3D &vec ) const {
	return Vec3D(
...
mxFORCEINLINE Matrix3 Matrix3::operator*( const Matrix3 &a ) const {
	INT i, j;
...
mxFORCEINLINE Matrix3 Matrix3::operator*( const FLOAT a ) const {
	return Matrix3(
...
mxFORCEINLINE Matrix3 &Matrix3::operator*=( const FLOAT a ) {
	mColumns[0].x *= a; mColumns[0].y *= a; mColumns[0].z *= a;
	mColumns[1].x *= a; mColumns[1].y *= a; mColumns[1].z *= a; 
	mColumns[2].x *= a; mColumns[2].y *= a; mColumns[2].z *= a;
...
mxFORCEINLINE Matrix3 &Matrix3::operator*=( const Matrix3 &a ) {
	INT i, j;
	const FLOAT *m2Ptr;
	FLOAT *m1Ptr, dst[3];
...
mxFORCEINLINE Vec3D operator*( const Vec3D &vec, const Matrix3 &mat ) {
	return mat * vec;
}

mxFORCEINLINE Matrix3 operator*( const FLOAT a, const Matrix3 &mat ) {
	return mat * a;
}

mxFORCEINLINE Vec3D &operator*=( Vec3D &vec, const Matrix3 &mat ) {
...
mxFORCEINLINE bool Matrix3::Compare( const Matrix3 &a ) const {
...
mxFORCEINLINE bool Matrix3::Compare( const Matrix3 &a, const FLOAT epsilon ) const {
...
mxFORCEINLINE bool Matrix3::operator==( const Matrix3 &a ) const {
	return Compare( a );
}
...
mxFORCEINLINE void Matrix3::SetZero( void ) {
	memset( mColumns, 0, sizeof( Matrix3 ) );
}

mxFORCEINLINE void Matrix3::SetIdentity( void ) {
	*this = mat3_identity;
}

mxFORCEINLINE bool Matrix3::IsIdentity( const FLOAT epsilon ) const {
	return Compare( mat3_identity, epsilon );
}

mxFORCEINLINE bool Matrix3::IsSymmetric( const FLOAT epsilon ) const {
	if ( fabs( mColumns[0][1] - mColumns[1][0] ) > epsilon ) {
		return false;
	}
	if ( fabs( mColumns[0][2] - mColumns[2][0] ) > epsilon ) {
		return false;
	}
	if ( fabs( mColumns[1][2] - mColumns[2][1] ) > epsilon ) {
		return false;
	}
	return true;
}

mxFORCEINLINE bool Matrix3::IsDiagonal( const FLOAT epsilon ) const {
	if ( fabs( mColumns[0][1] ) > epsilon ||
		fabs( mColumns[0][2] ) > epsilon ||
		fabs( mColumns[1][0] ) > epsilon ||
		fabs( mColumns[1][2] ) > epsilon ||
		fabs( mColumns[2][0] ) > epsilon ||
		fabs( mColumns[2][1] ) > epsilon ) {
		return false;
	}
	return true;
}

mxFORCEINLINE bool Matrix3::IsRotated( void ) const {
	return !Compare( mat3_identity );
}

mxFORCEINLINE void Matrix3::ProjectVector( const Vec3D &src, Vec3D &dst ) const {
	dst.x = src * mColumns[ 0 ];
	dst.y = src * mColumns[ 1 ];
	dst.z = src * mColumns[ 2 ];
}

mxFORCEINLINE void Matrix3::UnprojectVector( const Vec3D &src, Vec3D &dst ) const {
	dst = mColumns[ 0 ] * src.x + mColumns[ 1 ] * src.y + mColumns[ 2 ] * src.z;
}

mxFORCEINLINE bool Matrix3::FixDegeneracies( void ) {
...
mxFORCEINLINE bool Matrix3::FixDenormals( void ) {
...
mxFORCEINLINE FLOAT Matrix3::Trace( void ) const {
	return ( mColumns[0][0] + mColumns[1][1] + mColumns[2][2] );
}

mxFORCEINLINE Matrix3 Matrix3::OrthoNormalize( void ) const {
	Matrix3 ortho;
...
mxFORCEINLINE Matrix3 &Matrix3::OrthoNormalizeSelf( void ) {
...
mxFORCEINLINE Matrix3 Matrix3::Transpose( void ) const {
...
mxFORCEINLINE Matrix3 &Matrix3::TransposeSelf( void ) {
...
mxFORCEINLINE Matrix3 Matrix3::Inverse( void ) const {
	Matrix3 invMat;
...
mxFORCEINLINE Matrix3 Matrix3::InverseFast( void ) const {
	Matrix3 invMat;
...
mxFORCEINLINE Matrix3 Matrix3::TransposeMultiply( const Matrix3 &b ) const {
...
mxFORCEINLINE void TransposeMultiply( const Matrix3 &transpose, const Matrix3 &b, Matrix3 &dst ) {
...
mxFORCEINLINE Matrix3 SkewSymmetric( Vec3D const &src ) {
	return Matrix3(
		0.0f,	-src.z,	src.y,
		src.z,	0.0f,	-src.x,
		-src.y,	src.x,	0.0f
...
mxFORCEINLINE INT Matrix3::GetDimension( void ) const {
	return 9;
}

mxFORCEINLINE const FLOAT *Matrix3::ToPtr( void ) const {
	return mColumns[0].ToPtr();
}

mxFORCEINLINE FLOAT *Matrix3::ToPtr( void ) {
	return mColumns[0].ToPtr();
}
