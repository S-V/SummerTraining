...
mxBEGIN_STRUCT(Matrix4)
	mxMEMBER_FIELD(mRows[0]),
	mxMEMBER_FIELD(mRows[1]),
	mxMEMBER_FIELD(mRows[2]),
	mxMEMBER_FIELD(mRows[3]),
mxEND_REFLECTION
...
void Matrix4::SetRotationInDegrees( const Vec3D& rotation ) {
	SetRotationInRadians( rotation * mxDEG2RAD );
}
...
void Matrix4::SetRotationInRadians( const Vec3D& rotation ) {
	FLOAT cr;
	FLOAT sr;
	Float_SinCos( rotation.x, sr, cr );

	FLOAT cp;
	FLOAT sp;
	Float_SinCos( rotation.y, sp, cp );

	FLOAT cy;
	FLOAT sy;
	Float_SinCos( rotation.z, sy, cy );
...
Vec3D Matrix4::GetRotationInDegrees() const
{
	...
	if ( fabs( C ) > FLT_EPSILON )
	{
		...
		rotx = mat[2][2] * invC;
		roty = mat[1][2] * invC;
		X = Float_ATan2( roty, rotx ) * mxRAD2DEG;
		rotx = mat[0][0] * invC;
		roty = mat[0][1] * invC;
		Z = Float_ATan2( roty, rotx ) * mxRAD2DEG;
	}
	else
	{
		X = 0.0f;
...
{	
	...
	for ( UINT row = 0; row < 4; row++ ) {
		for ( UINT col = 0; col < 4; col++ ) {
			mat[row][col] = fabs( mRows[row][col] );
		}
	}
	return mat;
#else
	return Matrix4(
		fabs( mRows[0][0] ), fabs( mRows[0][1] ), fabs( mRows[0][2] ), fabs( mRows[0][3] ),
		fabs( mRows[1][0] ), fabs( mRows[1][1] ), fabs( mRows[1][2] ), fabs( mRows[1][3] ),
		fabs( mRows[2][0] ), fabs( mRows[2][1] ), fabs( mRows[2][2] ), fabs( mRows[2][3] ),
		fabs( mRows[3][0] ), fabs( mRows[3][1] ), fabs( mRows[3][2] ), fabs( mRows[3][3] )
	);
#endif
}
...
//
void Matrix4::BuildTransform( const Vec3D& vTranslation, const idQuat& qRotation, const Vec3D& vScale )
{
...
//	Matrix4::BuildTransform
//
void Matrix4::BuildTransform( const Vec3D& vTranslation, const idQuat& qRotation )
{
	*this = qRotation.ToMat4();
	this->SetTranslation( vTranslation );
}

idQuat Matrix4::ToQuat() const
{
