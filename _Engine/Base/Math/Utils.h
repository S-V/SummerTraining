...
template< typename T > mxFORCEINLINE T	Lerp( const T& a, const T& b, const T amount = 0.5f )	{ return a + (b - a) * amount; }

// bilinear interpolation
template< typename T > mxFORCEINLINE
T BiLerp(
...
{
	FLOAT A3 = Float_Pow( A, 3 );
	FLOAT A2 = Float_Pow( A, 2 );
...
inline T cosineInterpolate(T const &start, T const &end, FLOAT const t)
{
    ...
    {
            FLOAT mu2 = (1.0f - Float_Cos(t * M_PI)) / 2.0f;
 ...
 */
mxINLINE int FindRealQuadraticRoots( FLOAT a, FLOAT b, FLOAT c, FLOAT& x0, FLOAT& x1 )
{
	if( a == 0.0f )
	{
		// bx + c = 0
...
FLOAT CubeRoot( FLOAT x ) {
	return Float_Exp( FLOAT(1.0/3.0) * Float_Log(x) );
}
