...
const Vec3D Vec3D::vec3_neg_unit	(-1.0f,-1.0f,-1.0f );
...
FLOAT Vec3D::ToPitch( void ) const {
	...
	if ( ( x == 0.0f ) && ( y == 0.0f ) ) {
		...
		forward = ( FLOAT )Float_Sqrt( x * x + y * y );
		pitch = RAD2DEG( Float_ATan2( z, forward ) );
		if ( pitch < 0.0f ) {
			pitch += 360.0f;
		}
	}
...
const Polar3 Vec3D::ToPolar( void ) const {
	...
	return Polar3( Float_Sqrt( x * x + y * y + z * z ), yaw, -pitch );
}
...
void Vec3D::SLerp( const Vec3D &v1, const Vec3D &v2, const FLOAT t ) {
	...
	if ( ( 1.0f - cosom ) > LERP_DELTA ) {
		...
		sinom = Float_Sin( omega );
		scale0 = Float_Sin( ( 1.0f - t ) * omega ) / sinom;
		scale1 = Float_Sin( t * omega ) / sinom;
	} else {
		scale0 = 1.0f - t;
		scale1 = t;
	}
...
void Vec3D::ProjectSelfOntoSphere( const FLOAT radius ) {
	...
	FLOAT len = GetLength();
	if ( len  < rsqr * 0.5f ) {
		z = Float_Sqrt( rsqr - len );
	} else {
		z = rsqr / ( 2.0f * Float_Sqrt( len ) );
	}
}
