...
const AABB AABB::unit_cube		( Vec3D(-.5f,-.5f,-.5f), Vec3D(+.5f,+.5f,+.5f) );
...
mxDEFINE_CLASS( AABB );
mxBEGIN_REFLECTION( AABB )
	mxMEMBER_FIELD( mPoints[0] ),
	mxMEMBER_FIELD( mPoints[1] ),
mxEND_REFLECTION
...
FLOAT AABB::GetRadius( void ) const {
	...
	total = 0.0f;
	for ( i = 0; i < 3; i++ ) {
		b0 = (FLOAT)fabs( mPoints[0][i] );
		b1 = (FLOAT)fabs( mPoints[1][i] );
		if ( b0 > b1 ) {
			total += b0 * b0;
		} else {
			total += b1 * b1;
...
FLOAT AABB::GetRadius( const Vec3D &center ) const {
	...
	total = 0.0f;
	for ( i = 0; i < 3; i++ ) {
		b0 = (FLOAT)fabs( center[i] - mPoints[0][i] );
		b1 = (FLOAT)fabs( mPoints[1][i] - center[i] );
		if ( b0 > b1 ) {
			total += b0 * b0;
		} else {
			total += b1 * b1;
...
FLOAT AABB::PlaneDistance( const Plane3D &plane ) const
{
	...
	d2 = fabs( ( mPoints[1][0] - center[0] ) * plane.Normal()[0] ) +
			fabs( ( mPoints[1][1] - center[1] ) * plane.Normal()[1] ) +
				fabs( ( mPoints[1][2] - center[2] ) * plane.Normal()[2] );
...
FLOAT AABB::ShortestDistance( const Vec3D &point ) const {
	return Float_Sqrt( this->ShortestDistanceSquared( point ) );
}
		...
		if ( ( v1[i] > 0.0f && v2[i] < 0.0f ) || ( v1[i] < 0.0f && v2[i] > 0.0f ) ) {
			if ( ( 0.5f * (start[i] + end[i]) - origin[i] ) > 0.0f ) {
				...
				if( axis[i] * axis[i] < 1.0f ) {
					bounds[1][i] += Float_Sqrt( radiusSqr * ( 1.0f - axis[i] * axis[i] ) );
				}
			}
			else {
				bounds[0][i] = origin[i];
				if( axis[i] * axis[i] < 1.0f ) {
					bounds[0][i] -= Float_Sqrt( radiusSqr * ( 1.0f - axis[i] * axis[i] ) );
				}
...
const Vec3D* AABB::GetUnitCubeVertexNormals()
{
	...
	{
		Vec3D( -mxINV_SQRT_3,	-mxINV_SQRT_3,	-mxINV_SQRT_3	),
		Vec3D( mxINV_SQRT_3,	-mxINV_SQRT_3,	-mxINV_SQRT_3	),
		Vec3D( mxINV_SQRT_3,	mxINV_SQRT_3,	-mxINV_SQRT_3	),
		Vec3D( -mxINV_SQRT_3,	mxINV_SQRT_3,	-mxINV_SQRT_3	),
