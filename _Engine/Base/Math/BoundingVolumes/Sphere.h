...
//	Sphere
//
mxPREALIGN(16) struct Sphere : public CStruct
{
	Vec3D			Center;//mCenter
	FLOAT			Radius;//mRadius

public:
	mxDECLARE_CLASS( Sphere, CStruct );
	...
	Sphere			Transform( const Vec3D& translation, const idQuat& rotation, const FLOAT scale ) const;
	Sphere &		TransformSelf( const Vec3D& translation, const idQuat& rotation, const FLOAT scale );
...
mxFORCEINLINE Sphere::Sphere( void ) {
}

mxFORCEINLINE Sphere::Sphere( const Vec3D &point ) {
	Center = point;
	Radius = 0.0f;
}

mxFORCEINLINE Sphere::Sphere( const Vec3D &point, const FLOAT r ) {
	Center = point;
	Radius = r;
}

mxFORCEINLINE Sphere::Sphere( EInitZero ) {
	Center.SetZero();
	Radius = 0.0f;
}

mxFORCEINLINE Sphere::Sphere( EInitInfinity ) {
	Center.SetZero();
	Radius = BIG_NUMBER;
}

mxFORCEINLINE FLOAT Sphere::operator[]( const UINT index ) const {
	return ((FLOAT *) &Center)[index];
}

mxFORCEINLINE FLOAT &Sphere::operator[]( const UINT index ) {
	return ((FLOAT *) &Center)[index];
}

mxFORCEINLINE Sphere Sphere::operator+( const Vec3D &t ) const {
	return Sphere( Center + t, Radius );
}

mxFORCEINLINE Sphere &Sphere::operator+=( const Vec3D &t ) {
	Center += t;
	return *this;
}

mxFORCEINLINE bool Sphere::Compare( const Sphere &a ) const {
	return ( Center.Compare( a.Center ) && Radius == a.Radius );
}

mxFORCEINLINE bool Sphere::Compare( const Sphere &a, const FLOAT epsilon ) const {
	return ( Center.Compare( a.Center, epsilon ) && fabs( Radius - a.Radius ) <= epsilon );
}

mxFORCEINLINE bool Sphere::operator==( const Sphere &a ) const {
	return Compare( a );
}
...
mxFORCEINLINE void Sphere::Clear( void ) {
	Center.SetZero();
	Radius = -1.0f;
}

mxFORCEINLINE void Sphere::SetZero( void ) {
	Center.SetZero();
	Radius = 0.0f;
}

mxFORCEINLINE void Sphere::SetInfinity() {
	Center.SetZero();
	Radius = BIG_NUMBER;
}

mxFORCEINLINE void Sphere::SetOrigin( const Vec3D &o ) {
	Center = o;
}

mxFORCEINLINE void Sphere::SetRadius( const FLOAT r ) {
	Radius = r;
}

mxFORCEINLINE void Sphere::Set( FLOAT x, FLOAT y, FLOAT z, FLOAT radius ) {
	Center.x = x;
	Center.y = y;
	Center.z = z;
	Radius = radius;
...
mxFORCEINLINE const Vec3D &Sphere::GetOrigin( void ) const {
	return Center;
}

mxFORCEINLINE FLOAT Sphere::GetRadius( void ) const {
	return Radius;
}

mxFORCEINLINE bool Sphere::IsCleared( void ) const {
	return ( Radius < 0.0f );
}

mxFORCEINLINE bool Sphere::AddPoint( const Vec3D &p ) {
	if ( Radius < 0.0f ) {
		Center = p;
		Radius = 0.0f;
		return true;
...
mxFORCEINLINE bool Sphere::AddSphere( const Sphere &s ) {
	if ( Radius < 0.0f ) {
		Center = s.Center;
		Radius = s.Radius;
		return true;
...
mxFORCEINLINE Sphere Sphere::Expand( const FLOAT d ) const {
	return Sphere( Center, Radius + d );
}

mxFORCEINLINE Sphere &Sphere::ExpandSelf( const FLOAT d ) {
	Radius += d;
	return *this;
}

mxFORCEINLINE Sphere Sphere::Translate( const Vec3D &translation ) const {
	return Sphere( Center + translation, Radius );
}

mxFORCEINLINE Sphere &Sphere::TranslateSelf( const Vec3D &translation ) {
	Center += translation;
	return *this;
}

mxINLINE Sphere Sphere::Transform( const Vec3D& translation, const idQuat& rotation, const FLOAT scale ) const
{
	return Sphere(
...
mxFORCEINLINE Sphere &Sphere::TransformSelf( const Vec3D& translation, const idQuat& rotation, const FLOAT scale )
{
	*this = this->Transform( translation, rotation, scale );
}

mxFORCEINLINE bool Sphere::ContainsPoint( const Vec3D &p ) const {
...
mxFORCEINLINE bool Sphere::ContainsSphere( const Sphere& s ) const
{
	// find distance between centers
	const FLOAT dist = ( s.Center - Center ).GetLength();

	return (dist + s.Radius) < this->Radius;
}

mxFORCEINLINE bool Sphere::IntersectsSphere( const Sphere &s ) const {
...
mxFORCEINLINE void Sphere::FromPointTranslation( const Vec3D &point, const Vec3D &translation ) {
	Center = point + 0.5f * translation;
	Radius = Float_Sqrt( 0.5f * translation.LengthSqr() );
}

mxFORCEINLINE void Sphere::FromSphereTranslation( const Sphere &sphere, const Vec3D &start, const Vec3D &translation ) {
	Center = start + sphere.Center + 0.5f * translation;
	Radius = Float_Sqrt( 0.5f * translation.LengthSqr() ) + sphere.Radius;
}

mxFORCEINLINE void Sphere::FromPointRotation( const Vec3D &point, const Rotation &rotation ) {
	Vec3D end = rotation * point;
	Center = ( point + end ) * 0.5f;
	Radius = Float_Sqrt( 0.5f * ( end - point ).LengthSqr() );
}

mxFORCEINLINE void Sphere::FromSphereRotation( const Sphere &sphere, const Vec3D &start, const Rotation &rotation ) {
	Vec3D end = rotation * sphere.Center;
	Center = start + ( sphere.Center + end ) * 0.5f;
	Radius = Float_Sqrt( 0.5f * ( end - sphere.Center ).LengthSqr() ) + sphere.Radius;
}

mxFORCEINLINE void Sphere::AxisProjection( const Vec3D &dir, FLOAT &min, FLOAT &max ) const {
	FLOAT d;
	d = dir * Center;
	min = d - Radius;
	max = d + Radius;
...
mxFORCEINLINE bool Sphere::IsValid() const {
