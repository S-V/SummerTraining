...
bool OOBB::LineIntersection( const Vec3D &start, const Vec3D &end ) const {
	...
	if ( fabs( dir * axis[0] ) > extents[0] + ld[0] ) {
        return false;
	}

    ld[1] = fabs( lineDir * axis[1] );
	if ( fabs( dir * axis[1] ) > extents[1] + ld[1] ) {
        return false;
	}

    ld[2] = fabs( lineDir * axis[2] );
	if ( fabs( dir * axis[2] ) > extents[2] + ld[2] ) {
        return false;
	}

    Vec3D cross = lineDir.Cross( dir );

	if ( fabs( cross * axis[0] ) > extents[1] * ld[2] + extents[2] * ld[1] ) {
        return false;
	}

	if ( fabs( cross * axis[1] ) > extents[0] * ld[2] + extents[2] * ld[0] ) {
        return false;
	}

	if ( fabs( cross * axis[2] ) > extents[0] * ld[1] + extents[1] * ld[0] ) {
        return false;
	}
